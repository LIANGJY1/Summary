...
| 迁移时同步修改 | ① 源码/文档中的 `tools/screenshot/...` 命令示例改为 `project/honda27m-appstore-tools/screenshot/...`；② `compare_report.py` 新增实机图目录 `<模式>_split` 自动回退（修复分屏模式配对 0 的问题，见下文「_split 后缀约定」） |
...
```text
project/honda27m-appstore-tools/

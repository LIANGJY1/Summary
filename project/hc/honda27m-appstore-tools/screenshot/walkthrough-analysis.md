...
- **Mock 边界 Q2=A 零侵入**：仅改 `src/debug` + `mock/assets` + `project/honda27m-appstore-tools/screenshot`；业务文案等 `src/main` 只出 patch。见 ADR-0002。
...
- `project/honda27m-appstore-tools/screenshot/`：`capture_appstore_screenshots.py`（`clear→set→nav→wait→screencap→pull`）、`compare_report.py`（按 NNN 配对）、`report/` 已有 `index.html`。

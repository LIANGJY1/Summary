#!/usr/bin/env python3
"""
UI图 ↔ 实机图 配对并排 HTML 报告生成器。

按三位序号自动配对 UE 设计稿与车机实拍截图，生成自包含的并排对比页面，
供 UI走查 肉眼比对使用。零第三方依赖。

用法（与截图脚本共用「一键模式配置区」，默认跟随其顶部 CURRENT_VARIANT）：
    python3 project/honda27m-appstore-tools/screenshot/compare_report.py
    # 实机图取 screenshots/<当前模式>/（不存在而 <模式>_split 存在时自动改用后者，
    # 对齐分屏截图脚本的输出命名），UI图 取该模式 VARIANTS[mode].ref_dir，
    # 报告输出 report/<当前模式>/index.html，均互不覆盖。

可选覆盖（不影响截图脚本配置）：
    python3 project/honda27m-appstore-tools/screenshot/compare_report.py --ref-dir <UI图目录> --actual-dir <实机图目录> --out <html路径>
"""
...
DEFAULT_REF_DIR = Path.home() / "Documents/HC/UI/extracted_images/AppStore_全屏cn_D"
DEFAULT_OUT = Path(__file__).with_name("report") / "index.html"

# 与 capture_appstore_screenshots.py 共用「一键模式配置区」：改它顶部的 CURRENT_VARIANT，
# 本脚本未传参时自动跟随对应模式的 实机图目录 / UI图目录 / 报告输出目录。
try:
    import capture_appstore_screenshots as _capture_cfg
except Exception:
    _capture_cfg = None


def get_current_mode() -> tuple:
    """返回 (当前模式名, 该模式配置 dict)；截图脚本不可导入或未登记时回退 ('', {})。"""
    variant = getattr(_capture_cfg, "CURRENT_VARIANT", "") if _capture_cfg else ""
    variants = getattr(_capture_cfg, "VARIANTS", {}) if _capture_cfg else {}
    if not isinstance(variants, dict):
        variants = {}
    return variant or "", variants.get(variant, {})


def with_split_fallback(candidate: Path) -> Path:
    """目录不存在且同名 +_split 目录存在时改用后者。

    分屏截图脚本（capture_appstore_splitscreenshots.py）会在模式 output_dir 后追加
    _split 作为实际输出目录，本脚本的报告阶段需对齐该命名，否则配对恒为 0。
    配置目录本身存在时优先原样使用（显式配置优先于回退）。
    """
    if not candidate.exists():
        split_candidate = candidate.parent / (candidate.name + "_split")
        if split_candidate.exists():
            return split_candidate
    return candidate


def default_actual_dir() -> Path:
    """默认实机图目录：跟随模式 output_dir；缺失时退回 screenshots/<模式>；无模式信息时用旧版根目录。
    目标目录不存在而同名 +_split 目录存在时自动改用后者（见 with_split_fallback）。"""
    script_dir = Path(__file__).resolve().parent
    variant, entry = get_current_mode()
    configured = entry.get("output_dir")
    candidate = None
    if isinstance(configured, str) and configured.strip():
        p = Path(configured.strip())
        candidate = (p if p.is_absolute() else script_dir / p).resolve()
    elif variant:
        candidate = script_dir / "screenshots" / variant
    if candidate is None:
        return Path(__file__).with_name("screenshots")
    return with_split_fallback(candidate)


def default_ref_dir() -> Path:
    """默认 UI图目录：跟随截图脚本的 get_mode_ref_dir（显式 ref_dir > 模式名自动映射
    extracted_images 子目录），无法识别模式时回退内置默认目录。"""
    variant, _ = get_current_mode()
    if _capture_cfg is not None and hasattr(_capture_cfg, "get_mode_ref_dir"):
        resolved = _capture_cfg.get_mode_ref_dir(variant)
        if resolved:
...
def default_out_path() -> Path:
    """报告输出按模式分目录（report/<模式>/index.html），不同模式的报告互不覆盖。"""
    variant, _ = get_current_mode()
    if variant:
        return Path(__file__).with_name("report") / variant / "index.html"
    return DEFAULT_OUT
...
<head>
<meta charset="utf-8">
<title>UI走查 并排对比报告（含像素级差异）</title>
<style>
...
def render(rows: list, out_path: Path, ref_dir: Path, actual_dir: Path, mode: str = "") -> None:
...
def main() -> None:
    mode, _entry = get_current_mode()
    parser = argparse.ArgumentParser(description="UI图↔实机图 并排对比报告生成器（默认跟随截图脚本当前模式）")
    parser.add_argument("--ref-dir", type=Path, default=None,
                        help=f"UI图目录（默认跟随模式配置，当前回退值: {default_ref_dir()}）")
    parser.add_argument("--actual-dir", type=Path, default=None,
                        help=f"实机图目录，含分类子目录（默认: {default_actual_dir()}）")
    parser.add_argument("--out", type=Path, default=None,
                        help=f"输出 HTML 路径（默认: {default_out_path()}）")
    args = parser.parse_args()

    ref_dir = args.ref_dir or default_ref_dir()
    actual_dir = args.actual_dir or default_actual_dir()
    ...
    render(rows, out_path, ref_dir, actual_dir, mode)
    print(f"模式: {mode or '(未识别，走旧版默认)'}")

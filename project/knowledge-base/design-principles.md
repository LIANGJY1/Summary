...
## 边沿申请被拒不等于条件消失，状态型行为靠重算点自愈

**一句话**：事件驱动的动作系统里，"条件满足期间持续做某事"的行为只在条件变化沿申请一次；申请被裁决拒绝或独占后，事件已消费而条件仍成立——必须定义若干"重算点"对全部状态型行为按实时条件重新评估，否则行为永久饿死。

**核心矛盾**：竞合裁决的严格性要求"拒绝就是拒绝"（新申请失败，保护当前动作不被打断）；但拒绝消费掉的正是唯一的申请事件——之后条件一直成立却再无申请到来。裁决越严格，执行缺口越大。

**代码实例**（摘自 萌宠服务 `yadea_master/application/Launcher/.../pet/core/PetProtocol.kt`，事件仅沿触发）：

```kotlin
sealed class PetEvent {
    data class AudioPlay(val playing: Boolean) : PetEvent()  // 只在播放/暂停的瞬间出现
    data class PlugChanged(val state: Int) : PetEvent()
}
// 场景：充电动作占位时音乐开始播放 → 申请被拒绝 → 之后拔枪只有
// "充电结束"事件，没有任何"音乐还在播"的新沿 → 舞动永不出现
// 修复：重算点上不重放旧事件，而是按实时条件重新申请
fun evaluateAll(now: Conditions): List<PetCommand> =
    stateActions.filter { it.condition(now) && arbiter.allows(it) }
```

**思想提炼**：
- 边沿与状态是两种真值语义：沿回答"发生过什么"，状态回答"现在是否成立"；持续型行为必须由状态语义驱动，沿只负责发起申请。适用条件：行为存在明确的持续条件（播放中/插枪中/前台中）——一次性动作（播完即止）不受此缺口影响。
- 重算点要集中且幂等：把"该做什么"的评估收敛到少数确定时机（占用解除/信号变化/回前台），每次全量按实时条件重评估并仍过竞合；分散到各事件处理里就地补偿，漏一处就是一个饿死点。

## 异步的代价挂到生命周期关口偿还

**一句话**：把"快但可能丢"的操作异步化时，必须同时设计一个"慢但确定"的同步关口，挂在关键生命周期节点上强制清积压——异步的收益属于平时，异步的风险属于关口。

**核心矛盾**：apply 式异步写让调用方零等待，但进程随时可能被杀，盘上永远是旧值；等待会拖垮调用方，不等会丢数据——两边都是真需求。

**代码实例**（摘自 frameworks/base/core/java/android/app/QueuedWork.java + SharedPreferencesImpl.java）：

```java
// apply：内存即生效，落盘任务进队列，并登记一个"等本次落盘"的 finisher
QueuedWork.addFinisher(awaitCommit);      // awaitCommit 内部 latch.await()
QueuedWork.this.enqueueDiskWrite(mcr, postWriteRunnable);
// 生命周期关口（Activity.onPause、onReceive 结束、Service 停止）：
// 同步跑完全部队列 + 逐个运行 finisher——积压在这里一次性偿还
...
**一句话**：自愈机制（重启/回滚/兜底）自身可能陷入"处置→复发→再处置"的循环，必须配一层识别"自愈已失效"的检测，命中时切换到恢复域更强的兜底手段。

**核心矛盾**：兜底动作代价大（丢内存态、开机耗时），不能常态启用；但循环病灶在当前恢复域之外时，同层反复重试比一次高代价兜底危害更大——不识别循环就只能无限重复低效自愈。

**代码实例**（摘自 Android 13 `frameworks/base/services/core/java/com/android/server/Watchdog.java`）：

```java
// 滑动窗口内 fatal_count 次超时（sysprop，默认 0=关闭）即改走内核 panic：
// 杀 system_server 的软重启清不掉框架之外的病灶，断电级重启才可能恢复
if (!Build.IS_USER && isCrashLoopFound()
        && !WatchdogProperties.should_ignore_fatal_count().orElse(false)) {
    breakCrashLoop();  // SysRq 'c' 人为 panic
}
```

**思想提炼**：
- 自愈动作按"恢复域"排序分层（进程内重试 < 杀进程重启 < 版本回滚 < 整机重启），高频复发的失效要跨层升级而不是同层重试；适用条件：各层兜底代价有明确差异且复发频率可测量——代价无差别的幂等重试不需要分层。
- 兜底记账要与启动周期绑定（本例历史文件以 ro.boottime.zygote 为键，跨开机即作废），否则上一次开机的偶发会被算进本次窗口造成误判；适用条件：失效按时间窗判定——语义性失效（配置错）不适合时间窗模型。

## 跨边界引用是镜像计数，不是通知式回收

**一句话**：对象该不该活由全系统引用分布决定时，本地计数只是远端真账的投影——每次增减都要同步为跨边界的镜像命令，而不是攒一批通知等对方回收。

**核心矛盾**：对端对象的生死取决于多少个互不知晓的进程还持着它；任何一方崩溃、任何一次计数丢失都会造成悬挂引用或提前析构，而"等对方通知回收"在崩溃场景下根本不会送达。

**代码实例**（摘自 frameworks/native/libs/binder/BpBinder.cpp）：

```cpp
// 构造 → incWeakHandle（驱动弱计数+1，保证 handle 存在）；首个强引用 → incStrongHandle；
// 最后一个强引用 → decStrongHandle；析构 → decWeakHandle——sp/wp 每次增减即镜像命令
// flush 不及（命令积压在 mOut）时用临时本地引用垫住，送达后 processPostWriteDerefs 释放
```

**思想提炼**：
- 引用每变一次就发一条增量命令（可批量 flush），强/弱严格分批防析构重入——把远端生死当分布式一致性来对账；适用条件：远端生命周期不可信、本地可能随时崩溃；同进程内直接智能指针即可，别造两套计数。
- 强引用清零后对象仍存活，直到弱引用（含远端镜像）也清零——"最后一个理由消失才收"，死亡通知收件人这类弱持有者才能安全触碰；适用条件：存在需要穿越强引用归零窗口的弱持有方。


## 三类失败分通道，HTTP 错误不是异常

**一句话**：网络调用存在三种本质不同的失败——传输故障、HTTP 层错误、数据不符——塞进同一个通道（全抛异常或全当数据）都会逼调用方写错错误处理。

**核心矛盾**：调用方需要区别对待三种失败（重试/读错误体/修代码），但底层只有一条异常通道；把 404 当异常，调用方就拿不到错误体，把反序列化失败当正常数据，正常路径就得处处 try-catch。

**代码实例**（摘自 retrofit `retrofit2/Callback.java` 类头契约）：

```java
// 拿到 HTTP 响应（含 404/500）一律走 onResponse——HTTP 错误是数据不是异常；
// 网络/IO 失败或请求创建、反序列化抛异常才走 onFailure。
// 数据不符走 unchecked 异常，与 checked IOException（传输故障）在 catch 处可区分。
```

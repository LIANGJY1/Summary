...
- 以「学会这门语言」为骨架的学习体系笔记，可含生态应用章（如 `kotlin/11-Android架构.md`）；练习/实验项目（如 `C++/cpp_practice_project/`）
...
- 先找既有文件归并：主题相同/相邻并入既有文件；无处可归再新建，文件名用主题名（如 `java/泛型.md`），教学系列可用编号前缀（如 `kotlin/01-基础语法.md`），零散沉淀不编号。

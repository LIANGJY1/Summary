...
1. **收件箱协议（inbox）**：agent 把"待用户裁决的题目候选"写为仅含题目与答案的候选文件放入 `<库根>/atlas/inbox/`；Atlas 提供确认工作台，逐条 保留/编辑/丢弃 后落入 `questions.md`。闪卡由题库条目派生或手动创建。
...
```
建题库(粘贴题单/agent按主题生成→inbox确认) → 模拟面试(agent逐题提问,批改结果写回questions.md)
...
| 题库 | `<库根>/atlas/questions.md` | md 小节：题面/参考要点/复现状态 | 同上 |
...
- 描述：
  - **题库**：`atlas/questions.md`，每题一小节：题面/参考要点/作答与批改记录（追加式：原作答不改不删，批改直接追加）/状态（`未测→待复测→已稳定`）。三条建题途径统一确认门：粘贴题单导入、agent 按主题生成（经 inbox）、库内面试题 md 收录。

...
这三个维度里，最容易被误解的其实是“时钟驱动”。所以，文章下面先从最底层的物理屏幕与 VSync 硬件起源讲起，再顺着“硬件脉冲如何上传、A13 如何重建软件时钟、软件时钟如何驱动 App 与 SurfaceFlinger、最终如何上屏”的方向，按链路层层往上分析。

## 2. VSync 的硬件起源：为什么屏幕会有固定“心跳”
...
## 3. 从硬件脉冲到 SurfaceFlinger：VSync 是如何被上传的
...
**驱动侧（内核）**：Linux 内核的 DRM 驱动事先在内核里注册了这个中断号的处理函数（Interrupt Handler）。当 CPU 被打断并跳入这个处理函数时，DRM 驱动做的第一件事就是：读取当前 CPU 的**精确系统时间戳（CLOCK_MONOTONIC，纳秒级别）**。这个时间戳极其重要，它记录了 VSync 发生的绝对物理时间。随后，DRM 把这个带有时间戳的事件塞进内核的一个事件队列里。
...
到这里为止，系统只是“知道有一个物理 VSync 发生过”。但 Android 13 真正关键的设计点，不是简单接收这个中断，而是要解决：**这条上传链路本身有延迟，怎么让上层仍然按正确时间启动工作。**

## 4. Android 13 的关键设计：把硬件 VSync 重建成稳定的软件时钟
...
## 5. 同一个软件时钟，为什么还要拆成 VSync-App 和 VSync-SF

### 5.1 Phase Offset：流水线为什么必须错峰
...
## 6. App 为什么不会在每个 VSync 都被唤醒

上面说的是系统如何“造出正确的节拍”；下面就自然过渡到另一个问题：既然节拍一直存在，为什么 App 并不会每一拍都被唤醒？
...
## 7. 从触发到上屏的完整闭环：App、GPU、SurfaceFlinger、HWC 如何接力

前面讲的是“节拍如何产生、如何上传、如何在 A13 中被重建和分发”。下面把这些时钟驱动到真正“出图”的过程完整串起来。
...
## 8. 回过头看 HWC：它不只是“透传层”，而是决策中心

前一节已经在“从触发到上屏”的完整闭环里看到了 HWC 的位置，但从架构上，它的重要性值得单独拎出来再看一遍。

### 8.1 Hardware Composer (HWC) 的角色：不仅是透传，更是决策中心
...
## 9. 再回到应用侧：按需渲染与 WMS 的预测性测量

前面已经从 VSync 的角度解释了为什么 App 不会在每个节拍都被唤醒；这里再把应用侧与窗口管理侧的关联补齐，使整篇文章闭环。

...
| `project/` | 项目资料：`hc/`（车机 CloudOS/SDK 分析、问题解析流程框架）、`yadi/`（桌宠需求与设计）、`WMS Viewer/`（窗口树可视化工具）、`knowledge-base/`（会话知识库，内含 `language/` 编程语言笔记：Kotlin 系列、C++（含 Binder 源码笔记）、Java（反射、泛型）） |

...
2. **写入分流**：可迁移经验 → knowledge-base（按其 ROUTING 路由）；真实工作项目分析 → `project/knowledge-base/career/work-project-analysis/`；架构理解 → project-architecture（走 project-decoder）；skill 改动 → 改 `~/.agents/skills` 再同步。分流不确定 → 问用户，不猜。

...
图中接口只是各层向下使用的常见入口，不是强制路径：

1. 应用可以经 Bionic 直接发起文件系统调用；
2. SurfaceFlinger 可以直接对接 Composer HAL 和 DRM 显示子系统。

排查时按这个方向逐边界取证（每步取什么证据、线程状态怎么判读，见 Q5）：

1. 确定工作发生在哪个进程；
2. 确认它通过哪个接口（Binder/JNI/系统调用/HAL）进入下一个模块；
3. 看线程和调度状态。

**Q2: Android 五层架构之间是通过什么方式通信的，可以跨层通信吗？**

层与层之间通过各边界上的明确定义接口通信：同进程的跨界是进程内函数调用或 JNI，跨进程的跨界必须走显式 IPC（见 Q8）。跨层通信是允许的——五层是职责划分，不是调用管线，上层可以跳过中间层直达下层。
...
跳过中间层的直达例子见 Q1；一次请求实际穿过哪些层、怎么拆成控制命令与数据路径来判断，见 Q14。



**Q3: Android 架构层级与进程是什么关系？一个应用进程都包含哪些层级的代码？了解这些有什么用？**
...
3. **安全边界**：进程边界就是沙箱边界，每个应用默认独占一个 Linux UID 和一个进程（见 Q8），跨进程访问必须走 Binder 等显式 IPC。

**Q4: `system_server` 进程都包含哪些层级的代码？了解这些有什么用？**
...
**Q5: Android 实际工作场景中都有哪些架构问题？如何定位架构问题？**
...
**Q6: Zygote 在 Android 进程模型里扮演什么角色？为什么应用进程要用 fork 而不是各自独立启动？**
...
**Q7: HAL 有哪几种存在形态？Treble 之后 system 与 vendor 的边界靠什么维持兼容？**
...
**Q8: 16 KB 页大小、Mainline 模块化、VNDK 废弃——这三个近年架构边界分别改变了什么？**
...
**Q9: WindowManagerService 和 SurfaceFlinger 各自负责什么？为什么说它们是"层≠进程"的典型实例？**
...
**Q10: Android 的进程回收架构由哪些角色组成？杀、冻、压分别是谁在做什么？**
...
**Q11: 一个 Android 应用进程内部有哪几类固定线程角色？一次点击到上屏经过哪些线程？**
...
**Q12: 应用的所有请求都需要经过 Android 的五层架构吗？**
...
**Q14: 公共内核是什么？量产设备都会使用吗？厂商会修改什么、为什么？**

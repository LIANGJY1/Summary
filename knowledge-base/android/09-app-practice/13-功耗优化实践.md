...
**Q28: 应用退后台一段时间后 WebSocket 心跳超时被服务端断开、解冻后定时任务瞬间连发，甚至后台"无故"死亡——缓存进程冻结（Freezer）下发生了什么？怎么治理长连接？**

冻结是 Android 11+ 对缓存进程的默认治理：cgroup freezer 把整个进程暂停——没有 CPU、没有定时器、socket 不收包——进程还在，但一切主动行为停摆；冻结流程与资格判断的机制细节见 memory 册，冻结中的 Binder 事务语义见架构册 Binder 深化篇。

1. **三类现象**：心跳停导致服务端按超时断连；解冻瞬间积压的 Handler 消息与定时任务连发（用 `elapsedRealtime` 差值确认积压）；冻结中收到同步 Binder 事务的进程会被直接终止（`ApplicationExitInfo` 的 `REASON_FREEZER`）；
2. **确认**：`dumpsys activity processes` 看 frozen/pending 状态；`getApplicationExitInfo()` 看 reason；先区分"被冻结"与"被厂商后台限制"——两者取证入口不同（OEM 差异见 Q3–Q5）；
3. **长连接治理**：需要持续连接的场景用前台服务（豁免冻结，遵守 Q7–Q9 的 FGS 纪律）；或退后台主动断连、解冻后重连；推送走 FCM 高优先级消息，不依赖冻结期心跳；
4. **边界**：冻结不是功耗问题的万能解释——先排除 `BACKGROUND_RESTRICTED` 与厂商限制（Q4），再查冻结状态。

**Q29: Android 14 强制的 foregroundServiceType 有哪些？从 BOOT_COMPLETED 能拉起哪几种前台服务？**
...
1. **超时语义**（超时归因的详细展开见 [性能册 ANR 篇](../07-performance/03-ANR.md) Q6）：shortService 约 3 分钟且不可重入；Android 15 起 targetSdk 35+ 的 dataSync/mediaProcessing 在 24 小时窗口内累计 6 小时，`onTimeout()` 后数秒内必须 stop，否则抛 `ForegroundServiceDidNotStopInTimeException`——常见误传是"Android 14 六小时"，6 小时是 Android 15 行为；

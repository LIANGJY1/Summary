...
**Q1: kotlin 中的区间是什么？有什么用？java 中有吗？？**
...
**Q3: 用 `object :` 实现单方法 Java 接口时，为什么 IDE 建议转换为 lambda？转换有什么前提？**
...
**Q7: Kotlin API 使用 `(Boolean) -> Unit` 作为回调时，Java 为什么需要返回 `Unit.INSTANCE`，什么时候应改用 `fun interface`？**
...
**Q9: 调用最后一个参数为函数类型的函数时，为什么 lambda 可以移到圆括号外，甚至省略圆括号？**

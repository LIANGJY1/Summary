...
**Q2: Kotlin API 使用 `(Boolean) -> Unit` 作为回调时，Java 为什么需要返回 `Unit.INSTANCE`，什么时候应改用 `fun interface`？**
...
**Q3: 读到 `StringBuilder.() -> Unit` 时应该怎样理解？它与 `(StringBuilder) -> Unit` 有什么区别？**
...
**Q4: 用 `object :` 实现单方法 Java 接口时，为什么 IDE 建议转换为 lambda？转换有什么前提？**
...
**Q5: 看到 `private fun vehicleService(): VehicleService = BaseManager.getInstance(VehicleService::class.java)!!` 时，应该怎样拆解语法，`vehicleService` 又是不是通用命名？**
...
**Q7: `listOf` 返回的集合为什么不能调用 `add`？Kotlin 的只读 `List` 是否等于不可变集合？**
...
**Q8: 调用最后一个参数为函数类型的函数时，为什么 lambda 可以移到圆括号外，甚至省略圆括号？**

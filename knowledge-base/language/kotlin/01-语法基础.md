...
**Q4: 调用最后一个参数为函数类型的函数时，为什么 lambda 可以移到圆括号外，甚至省略圆括号？**
...
**Q5: 创建可变 Map 时，`mutableMapOf`、`linkedMapOf` 与 `HashMap` 分别是否保证遍历顺序，应该怎样选择？**
...
**Q6: 使用列表大小生成下标区间时，为什么 `0..list.size` 会多遍历一次并导致越界？**
...
**Q7: kotlin 中的区间是什么？有什么用？java 中有吗？？**
...
**Q8: 接收者可能为 `null`，调用结果也可能为 `null` 时，如何用安全调用与 Elvis 运算符统一提供默认值？**
...
**Q9: 循环处理多个元素时，为什么不应在每轮都执行与循环变量无关的 `filter`？**
...
**Q10: 用 `object :` 实现单方法 Java 接口时，为什么 IDE 建议转换为 lambda？转换有什么前提？**
...
**Q11: `listOf` 返回的集合为什么不能调用 `add`？Kotlin 的只读 `List` 是否等于不可变集合？**
...
**Q13: 使用 `Int` 计算百分比时，为什么 `battery / total * 100` 可能得到 `0`，应该怎样保留正确精度？**
